...
```{r}
# Read Data
data <- read_csv("data_input/trainori.csv")
```
...
```{r}
submission <- read_csv("data_input/submission.csv") %>% 
  mutate(visitor = model_revert)
```

```{r}
write.csv(submission,"data_input/submission.csv")
```
